 ...
 */
$modalHTML = '';
$uri = new Uri('index.php?option=com_solidres&view=customers&layout=modal&tmpl=component&required=0');
...
$inputAttributes = array(
	'type' => 'text', 'id' => $id, 'class' => 'form-control field-solidres-customer-input-name', 'value' => $this->escape($userName)
);
if ($class)
{
...
if (!$readonly)
{
	...
	Factory::getDocument()->getWebAssetManager()
		->useScript('com_solidres.field-customer');
}
?>
<?php // Create a dummy text field with the username. ?>
<solidres-field-customer class="field-solidres-customer-wrapper"
                   url="<?php echo (string) $uri; ?>"
                   modal=".modal"
                   modal-width="100%"
                   modal-height="400px"
                   input=".field-solidres-customer-input"
                   input-name=".field-solidres-customer-input-name"
	...
	<?php if (!$readonly) : ?>
		<input type="hidden" id="<?php echo $id; ?>_id" name="<?php echo $name; ?>" value="<?php echo $this->escape($value); ?>"
		       class="field-solidres-customer-input <?php echo $class ? (string) $class : ''?>"
		       data-onchange="<?php echo $this->escape($onchange); ?>">
		<?php echo $modalHTML; ?>
	<?php endif; ?>
</solidres-field-customer>
